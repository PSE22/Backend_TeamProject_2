<template>
  <div class="container mt-3 board-detail-container">
    <div class="row mb-3 board-button">
      <button class="col-1 btn btn-warning me-2" @click="goClubEdit(bocode, smcode, boardId)">
        수정
      </button>
      <button class="col-1 btn btn-danger" data-bs-toggle="modal" data-bs-target="#delete">삭제</button>
    </div>
    <!-- 삭제 경고 Modal -->
    <div
      class="modal fade"
    ...
    <div class="card mb-3">
      <!-- 글 제목 부분 -->
      <div class="card-header text-white">동호회게시판 <i class="bi bi-caret-right"></i> {{ cmcd?.cmCdName }} <i class="bi bi-caret-right"></i> {{ smcmcd?.cmCdName }}</div>
      <div class="card-body">
...
export default {
  ...
  methods: {
    ...
    async deleteRecommend() {
      try {
        let response = await BoardDetailService.deleteRecommend(
          this.boardId,
          this.member.memberId
        );
        console.log(response.data);
