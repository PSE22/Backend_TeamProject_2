    ...
    <div class="container mt-3 board-detail-container">
      ...
      <div class="card mb-3">
        ...
        <div class="card-body">
          ...
          <div class="d-flex justify-content-start">
            <!-- 추천 아이콘 -->
            <div v-if="recommendIcon" @click="toggleRecommend" class="me-3" type="button"><i class="bi bi-hand-thumbs-up"></i> {{ recommendCnt }} </div>
            <div v-else class="me-3" type="button"><i class="bi bi-hand-thumbs-up-fill"></i> {{ recommendCnt }}</div>
            <!-- 댓글 아이콘 -->
            <div class="me-3" type="button"><i class="bi bi-chat-text"></i> {{ replyCount }} </div>
            <!-- 신고 아이콘 -->
            <div class="me-3" type="button" data-bs-toggle="modal" data-bs-target="#reportModal"><i class="bi bi-exclamation-triangle"></i>신고</div>
          </div>
        </div>
      </div>
      <!-- Modal -->
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              <div class="mb-2 row">
                ...
                <div class="col-sm-10">
                  <input type="text" readonly class="form-control-plaintext" id="staticWriter" v-model="board.memberName">
                </div>
              </div>
              <div class="mb-2 row">
                <label for="staticTitle" class="col-sm-2 col-form-label">내용</label>
                <div class="col-sm-10">
                  <input type="text" readonly class="form-control-plaintext" id="staticTitle" v-model="board.boardTitle">
                </div>
              </div>
              <div class="mb-2">
                <hr />
                <form class="was-validated">
                  <textarea rows="10" class="form-control" placeholder="신고 사유를 입력하세요." required v-model="reportReason"></textarea>
      ...
      <div class="reply-content mb-3">
        <ul v-for="(data, index) in reply" :key="index" class="list-group mb-3">
          ...
          <li v-if="!data.isEditing" class="list-group-item">
            ...
            <div class="d-flex justify-content-between mt-2">
              ...
              <div>
                ...
                <button class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#reportReplyModal" @click="openReplyReport(data)">
            ...
            <div v-if="showWriteReReply && parentId === data.replyId" class="card mb-3 mt-3">
              <div class="card-body">
                ...
                <textarea v-model="reReplyTextarea" placeholder="대댓글을 남겨보세요" class="form-control mb-2 reply-content"></textarea>
          ...
          <li v-if="data.isEditing" class="list-group-item">
            <div class="reply-name">{{ data.memberName }} </div>
            <textarea v-model="data.reply" placeholder="댓글을 남겨보세요" class="form-control mb-2 reply-content"></textarea>
            <div v-if="data.fileUrl" class="d-flex">
              <!-- 파일첨부 -->
              <input class="form-control file-upload-input mb-2" type="file" ref="replyFile" @change="selectReplyFile" /> {{ data.fileUrl }}
            </div>
            <div class="d-flex justify-content-between">
              <button class="btn btn-secondary" @click="cancelReplyUpdate(data.replyId)">취소</button>
              <button class="btn btn-secondary" @click="updateReply(data)">등록</button>
            </div>
          </li>
  
          <!-- 대댓글 -->
          <li v-for="(reReply, reIndex) in data.reReplies" :key="reIndex" class="list-group-item reReply-container">
            ...
            <div class="d-flex justify-content-between mt-2">
              ...
              <div>
                ...
                <button class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#reportReReplyModal" @click="openReplyReport(reReply)">
                  <i class="bi bi-exclamation-triangle"></i>신고
                </button>
              </div>
            </div>
      ...
      <div class="card mb-3">
        <div class="card-body">
          ...
          <div class="d-flex justify-content-between">
            <!-- 파일첨부 -->
            <input class="form-control file-upload-input" type="file" ref="replyFile" @change="selectReplyFile" />
    ...
    methods: {
      ...
      async retrieveReply() {
        try {
          ...
          this.reply = response.data.map(r => ({ ...r, isEditing: false }));
